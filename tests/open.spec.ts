...
describe('spawn wrappers', () => {
  it('resolves on spawn for a browser open', async () => {
    ...
    expect(spawnMock).toHaveBeenCalledWith(expected.command, expected.args, { stdio: 'ignore', windowsHide: true })
    child.emit('spawn')
  ...
  it('reveals a file and a folder in the file manager', async () => {
    ...
    expect(spawnMock).toHaveBeenLastCalledWith(fileExpected.command, fileExpected.args, { stdio: 'ignore', windowsHide: true })
    ...
    expect(spawnMock).toHaveBeenLastCalledWith(dirExpected.command, dirExpected.args, { stdio: 'ignore', windowsHide: true })
    dirChild.emit('spawn')
    await revealDir
  })
  ...
  it('treats a missing reveal target as a folder', async () => {
    ...
    expect(spawnMock).toHaveBeenLastCalledWith(expected.command, expected.args, { stdio: 'ignore', windowsHide: true })
    child.emit('spawn')
