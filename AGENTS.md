...
- **analyze 端点是 ctx 感知例外**：`一键AI分析`（`POST .../analyze`）需要在 index.ts 路由层直接处理（依赖可选的 `workspaceRegistry` / `agents` / `agentDefaultModel` / `agentPresets` 服务，经 `ctx.get` 读取，缺省时返回 500 信封或退化为无模型/无 preset 选项）；其编排逻辑 `runAnalysis` 独立导出可单测，不走 handler.ts 纯核心。分析会话必须带上宿主默认模型（`agentDefaultModel.currentSelection()` → `agentOptions`），否则系统提示词里的 `{{model}}` 无值、首轮组装即失败；还必须加入默认 agent preset（`agentPresets.resolve(undefined)` → `setup` 里 `mount`，并把 id 记入 `meta.agentPreset`），否则会话只有本插件的浏览器工具、没有 standard 工具集——行为必须与手动创建会话一致。
