...
- **analyze 端点是 ctx 感知例外**：`一键AI分析`（`POST .../analyze`）需要在 index.ts 路由层直接处理（依赖可选的 `workspaceRegistry` / `agents` / `agentDefaultModel` 服务，经 `ctx.get` 读取，缺省时返回 500 信封或退化为无模型选项）；其编排逻辑 `runAnalysis` 独立导出可单测，不走 handler.ts 纯核心。分析会话必须带上宿主默认模型（`agentDefaultModel.currentSelection()` → `agentOptions`），否则系统提示词里的 `{{model}}` 无值、首轮组装即失败。
