...
- **面包屑导航**——可点击的面包屑逐级返回任意上级目录。
- **视图切换**——列表/卡片（网格）视图一键切换。
- **三种显示尺寸**——工具栏按钮（显示当前档位）循环切换「视图大小」（居中窗口，默认）→「整个视图」（铺满视口）→「全屏」（浏览器全屏）。
- **侧边栏**——项目目录、快速访问（桌面/下载/文档…）、自定义根与磁盘驱动器。
- **搜索**——顶栏搜索，支持递归模式（命中上限 200）。
- **本地打开**——本机模式下右下角悬浮按钮，一键用系统文件管理器打开当前目录。
- **一键AI分析**——右键文件/文件夹 → 输入需求 → dsh 为该目录创建工作区并唤醒新会话的智能体进行分析（仅本机模式）。分析会话使用宿主默认模型，并加入与手动创建会话相同的 agent preset，因此拥有完整的 standard 工具集（read/write/glob/grep、shell、goal、web 搜索等），而不只是浏览器工具。
...
- **`/files` 输入框命令**——客户端斜杠命令：在**用户正在使用的浏览器**中新标签页打开文件浏览器并定位会话工作区，无头/远程部署下依然可用（宿主侧命令在无显示环境下打不开浏览器）。
...
- 在对话框输入 `/files` 并从斜杠菜单选取：文件浏览器在**你正在使用的浏览器**的新标签页打开并定位当前会话工作区——无头服务器/远程部署请走这条路（宿主命令 `/dsh-agfs` 保留，行为不变）。
...
- **`openOnCommand` 在宿主机上产生副作用**——浏览器在运行 dsh 的机器上打开，loopback 部署正确，对远程客户端有违直觉；远程部署请改用输入框 `/files`（在客户端浏览器打开，见「使用」）。
...
发布由 tag 驱动：推 `v*` tag 会触发 GitHub Actions（`.github/workflows/publish.yml`）自动发布到 npm。**tag 完全手动触发——脚本不会自动打 tag**：

```sh
pnpm run release                     # 仅查看发布状态（不写入）
pnpm run release -- --do             # 立即发版（递增 patch）：提交 + 打 tag + 推送
pnpm run release -- --do --version 0.2.0   # 按指定版本发版
pnpm run release -- --do --dry-run   # 预览（不写入、不推送）
```

`scripts/release.mjs` 无参数时打印当前版本、上次 `v*` tag 及自其以来的提交数；带 `--do` 时递增版本、提交 `chore: release vX.Y.Z`、创建 tag 并推送 `main` 与 tag。仓库需配置 `AGFS` secret（带 bypass-2FA 的细粒度 npm 发布 token）。本包把已发布的 `@deepseek-ai/dsh-*` 工具链包声明为 peer 依赖，发布安装可从 npm 解析。
